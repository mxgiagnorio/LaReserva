...
export default function ScorersTable({ id }) {
  ...
  useEffect(() => {
    axios
      .get(
        `http://lareserva-production.up.railway.app/scorers?tournament=${id}&limit=99`
      )
      .then((data) => setScorersInfo(data.data));
  }, [userDetail]);

  const handleAddGoal = (e) => {
    dispatch(addGoal(e.target.value)).then(() =>
      axios
        .get(
          `http://lareserva-production.up.railway.app/scorers?tournament=${id}&limit=99`
        )
  ...
  const handleQuitGoal = (e) => {
    dispatch(quitGoal(e.target.value)).then(() =>
      axios
        .get(
          `http://lareserva-production.up.railway.app/scorers?tournament=${id}&limit=99`
        )
    ...
    <table className="w-full relative bg-white text-gray-800 shadow shadow-gray-700">
      ...
      {scorersInfo[0] ? (
        scorersInfo.map((player, index) => {
          return (userDetail.admin === false || userDetail.admin === "dc") &&
            index > 8 ? null : (
            <tr
              key={`trScr${index}`}
