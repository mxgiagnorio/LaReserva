...
export default function ModifyTournament() {
  ...
  useEffect(() => {
    axios
      .get(`https://lareserva-production.up.railway.app/tournaments/${id}`)
      .then((data) =>
        setInput({
          name: data.data.name,
  ...
  function validateName(name) {
    if (name.length <= 0)
      return setErrors({ ...errors, name: "Este campo no puede estar vacio." });
    if (/[^A-Za-z 0-9]/g.test(name))
      return setErrors({
        ...errors,
        name: "Caracteres especiales no admitidos.",
      });
  ...
  function validateAmount(amount) {
    if (amount.length < 1)
      return setErrors({
        ...errors,
        amountOfTeams: "Este campo no puede estar vacio.",
      });
    if (amount < 8)
      return setErrors({ ...errors, amountOfTeams: "Minimo de 8 equipos." });
    if (amount > 40)
      return setErrors({ ...errors, amountOfTeams: "Maximo de 40 equipos." });
    return setErrors({ ...errors, amountOfTeams: "" });
  }

  function validateGenre(genre) {
    if (genre.length > 14)
      return setErrors({ ...errors, genre: "Seleccione un genero." });
    return setErrors({ ...errors, genre: "" });
  }

  function validateCategory(category) {
    if (category.length > 14)
      return setErrors({ ...errors, category: "Seleccione una categoria." });
  ...
  function validateDescription(description) {
    ...
    if (description.length > 200)
      return setErrors({ ...errors, description: "200 caracteres maximo." });
  ...
  function validateDateInit(dateInit) {
    ...
    if (new Date(dateInit) < today)
      return setErrors({
        ...errors,
        dateInit: "La fecha de inicio no puede ser en el pasado o hoy mismo.",
      });

    if (input.dateFinish && new Date(input.dateFinish) <= new Date(dateInit)) {
      //Si hay un fecha de inicio definida
      return setErrors({
        ...errors,
        dateInit:
          "La fecha de inicio no puede ser pasada o igual a la de finalizacion.",
      });
    }
  ...
  function validateDateFinish(dateFinish) {
    ...
    if (new Date(dateFinish) < today)
      return setErrors({
        ...errors,
        dateFinish:
          "La fecha de finalizacion no puede ser en el pasado o hoy mismo.",
      });

    if (input.dateInit && new Date(input.dateInit) >= new Date(dateFinish)) {
      //Si hay un fecha de inicio definida
      return setErrors({
        ...errors,
        dateFinish:
          "La fecha de finalizacion no puede ser anterior o igual a la de inicio.",
      });
    }
  ...
  function handleSubmit(e) {
    ...
    if (
      Object.keys(errors).some((element) => errors[element] !== "") ||
      Object.keys(input).some((element) => input[element] === "")
    ) {
      setPopUpError({
        title: "Error!",
  ...
  return (
    <div>
      ...
      <div className={styles.mainWrapper}>
        ...
        <form
          className="w-4/6 bg-gray-100 p-3 shadow shadow-gray-700 flex 
				flex-col items-center min-w-[280px]"
        >
          <div
            className="w-5/6 flex flex-col lg:flex-row items-center lg:items-end 
          ...
          >
            ...
            >
              {errors.dateFinish ? errors.dateFinish : errors.dateInit}
            </div>
          </div>
          <div
            className="w-5/6 flex flex-col lg:flex-row items-center lg:items-end 
          ...
          >
            ...
            <div
              className={
                errors.description ? styles.error_visible : styles.error_hidden
              }
            >
              {errors.description}
            </div>
        ...
        <div className="w-full flex justify-center py-10">
          <button
            className="flex items-center justify-center bg-green-500 w-[200px] h-[70px]
