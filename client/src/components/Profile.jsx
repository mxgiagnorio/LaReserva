...
export default function Profile() {
  ...
  useEffect(() => {
    if (!isLoading && user && actualUser.username === undefined) {
      ...
      axios
        ...
        .then((info) => {
          ...
          ); //Hago un dispatch guardando la info que ahora App.js podra acceder
        })
        .catch((err) => {
          axios
            ...
            .then((info) => {
              ...
              dispatch(
                setActualUser(userInfo.name, userInfo.ban, userInfo.admin)
              );
      ...
      {!isLoading &&
        user && ( //PONER ACA LAS COSAS QUE SE TIENEN QUE VER SI ESTAS CONECTADO
          <div className="flex items-center relative" ref={wrapperRef}>
            <Link
              to={actualUser.admin ? "/admin" : "/panel"}
            ...
            >
              ...
              <p className="font-medium ml-3 hover:text-green-700 duration-300">
                {user.nickname}
              </p>
            </Link>

            <BiDownArrow
              className="ml-3 cursor-pointer text-xl hover:text-green-700 duration-300"
              onClick={() => setVisibility(!visibility)}
            />

            <div
              className="bg-gray-300 flex flex-col items-start 
            ...
            >
              {actualUser.admin === true ? (
                <Link
                  to="/admin"
                  className="mb-2 hover:text-green-700 duration-300"
                >
                  Panel admin
                </Link>
              ) : (
