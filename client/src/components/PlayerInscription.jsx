...
export default function PlayerInscription() {
  ...
  useEffect(() => {
    axios
      .get("http://lareserva-production.up.railway.app/tournaments/panel")
      .then((res) => {
        setActiveTournaments(
          res.data.filter(
            (tournament) => tournament.state === "Proximo" && tournament.enabled
          )
        );
      })
      .catch(() => {
        console.error("No se encontro ningun torneo.");
      });
  }, []);
  ...
  function validateTournament(tournament) {
    if (tournament === "null")
      return setErrors({
        ...errors,
        tournament: "Debe seleccionar un torneo.",
      });
  ...
  function validateTeamName(name) {
    if (name.length < 4)
      return setErrors({ ...errors, teamName: "Minimo de 4 caracteres." });
    if (name.length > 40)
      return setErrors({
        ...errors,
        teamName: "El nombre no puede tener mas de 40 caracteres.",
      });
  ...
  function validatePlayerName(name) {
    if (name.length < 2)
      return setErrors({
        ...errors,
        playerName: "El nombre debe tener al menos dos letras.",
      });
    if (name.length > 15)
      return setErrors({
        ...errors,
        playerName: "El nombre no puede tener mas de 15 caracteres.",
      });
    if (!/^[A-Za-z]+$/.test(name))
      return setErrors({
        ...errors,
        playerName: "El nombre solo puede contener letras.",
      });
  ...
  function validatePlayerSurname(surname) {
    if (surname.length < 2)
      return setErrors({
        ...errors,
        playerSurname: "El apellido debe tener al menos dos letras.",
      });
    if (surname.length > 15)
      return setErrors({
        ...errors,
        playerSurname: "El apellido no puede tener mas de 15 caracteres.",
      });
    if (!/^[A-Za-z]+$/.test(surname))
      return setErrors({
        ...errors,
        playerSurname: "El apellido solo puede contener letras.",
      });
    return setErrors({ ...errors, playerSurname: "" });
  }

  function validateDni(dni) {
    if (!/^\d+$/.test(dni) && dni.length > 0)
      return setErrors({
        ...errors,
        playerDni: "Este campo solo acepta numeros.",
      });
    if (dni.length !== 8)
      return setErrors({
        ...errors,
        playerDni: "El dni debe tener 8 digitos.",
      });
  ...
  function errorMsg(errorName) {
    return (
      errors[errorName] && (
        <div
          className="bg-red-700 absolute right-0 font-medium p-1 rounded-lg 
    ...
    switch (phase) {
      case 0:
        if (
          ...
          setPopUpErrors({
            title: "Error!",
            msg: "Complete la informacion correctamente antes de enviar.",
          });
        } else {
      ...
      case 8: //Aca se termina todo
        if (
          Object.keys(errors).some((error) => errors[error] !== "") ||
          !inputs.playerName[phase - 1] ||
          !inputs.playerSurname[phase - 1] ||
          !inputs.playerDni[phase - 1]
        ) {
          //Si hay algun error
          setPopUpErrors({
            title: "Error!",
          ...
          setPopUpErrors({
            title: "Listo!",
            msg: "Todos los jugadores cargados correctamente.Sera redirigido a la pasarela de pago.",
          });
        }
        break;
      default:
        if (
          ...
          setPopUpErrors({
            title: "Exito!",
            msg: "Jugador cargado correctamente, por favor complete los datos del siguiente jugador.",
          });
          setPhase(phase + 1);
        }
        break;
  ...
  return (
    <div className="flex flex-col justify-between min-h-screen">
      ...
      <div className="flex flex-col items-center">
        <h1 className="text-green-500 font-bold text-4xl my-6">
          {phase === 0 ? "Inscribi a tu equipo" : `Jugador ${phase}`}
        </h1>

        {phase === 0 ? (
          <form className="bg-gray-100 w-5/6 p-3 min-w-fit shadow shadow-gray-700">
            <div
              className="flex justify-between flex-col lg:flex-row items-center 
            ...
            >
              ...
              >
                ...
                {activeTournaments?.map((tournament, index) => {
                  return (
                    <option key={`opt${index}`} value={tournament.id}>
                      {tournament.name}
                    </option>
                  );
                })}
            ...
            <div
              className="flex justify-between flex-col lg:flex-row items-center 
					lg:items-end my-3 relative h-[125px] lg:h-[100px] min-w-fit"
            >
              <label htmlFor="" className="text-3xl font-bold text-green-500">
                Nombre del equipo:
              </label>
              <input
                type="text"
            ...
            <div className="bg-gray-200 text-center shadow shadow-gray-700 p-1 mt-[60px] mb-[10px]">
              <p className="text-xl font-bold">
                Por favor, completá con los datos REALES de los jugadores.
              </p>
              <p className="text-xl">
                Recordá que los datos serán pedidos por el personal de la cancha
            ...
            <div className="w-full flex justify-center mt-[50px] mb-[20px]">
              <button
                className="bg-green-500 w-[200px] h-[60px] rounded-full 
