...
export default function PanelUser() {
  ...
  const handleDeleteUser = () => {
    ...
    logout({ returnTo: "https://la-reserva.vercel.app/home" });
  };

  useEffect(() => {
    if (user.email) {
      axios
        .get(`https://lareserva-production.up.railway.app/users/historial`)
        .then((res) => {
          ...
          filteredRes[0].teams.map((teams) => {
            promisifiedArray.push(
              axios.get(
                `http://lareserva-production.up.railway.app/teams?name=${teams.name}`
              )
            );
          });

          Promise.all(promisifiedArray).then((res) => {
          ...
          <div className="w-full flex flex-col items-center justify-start mt-6 mb-3 h-full">
            {tournaments.length !== 0 ? (
              ...
              <p className="text-xl font-medium">
                {"Todavia no te has unido a ningun torneo :("}
              </p>
            )}
          </div>
        </div>
